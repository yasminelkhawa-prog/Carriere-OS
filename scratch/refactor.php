...
$filePath = 'resources/views/candidates/index_backup.blade.php';
$content = file_get_contents($filePath);

// We want to wrap the contents inside the `@else` block inside an `@if(request()->filled('application_id'))`.
// The structure is:
...
if ($elsePos === false) {
    die("Could not find @else block\n");
}

$endIfPos = strrpos($content, "@endif\n</x-shell-layout>");
if ($endIfPos === false) {
    die("Could not find @endif block\n");
}

$startToElse = substr($content, 0, $elsePos + 5);
$innerContent = substr($content, $elsePos + 5, $endIfPos - ($elsePos + 5));
...
file_put_contents('resources/views/candidates/index.blade.php', $startToElse . $newBlock);
echo "Success!\n";
