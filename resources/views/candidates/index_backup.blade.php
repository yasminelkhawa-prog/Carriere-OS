    ...
    <div class="space-y-4 pb-28">
        ...
        @else
            <div class="grid gap-4 xl:grid-cols-[24rem_minmax(0,1fr)]">
                <x-glass-card :title="__('candidates.list.title')" body-class="flex min-h-0 flex-1 flex-col" class="min-w-0 flex flex-col p-0 2xl:h-[calc(100vh-11rem)] 2xl:overflow-hidden">
                    <form method="GET" action="{{ route('candidates.index') }}" class="space-y-3 border-b border-white/60 bg-white/55 px-3 py-3">
                        ...
                        <x-form-field :label="__('candidates.filters.search')" name="q">
                        ...
                        <div class="grid gap-3 md:grid-cols-2">
                            <x-form-field :label="__('candidates.filters.job')" name="job_id">
                            ...
                            <x-form-field :label="__('candidates.filters.stage')" name="stage_id">
                                <select name="stage_id" data-placeholder="{{ __('candidates.filters.stage_placeholder') }}" class="w-full rounded-xl border border-aura-200/40 bg-white/80 px-3 py-2 text-sm">
                                    <option value="">{{ __('candidates.filters.stage_placeholder') }}</option>
                                    @foreach($stages as $stage)
                                        <option value="{{ $stage->id }}" @selected(($filters['stage_id'] ?? null) === $stage->id)>{{ $stage->stage_label }}</option>
                                    @endforeach
                                </select>
                            </x-form-field>
                        </div>
                        ...
                        <div class="grid gap-3 md:grid-cols-2">
                            <x-form-field :label="__('candidates.filters.status')" name="status">
                            ...
                            <x-form-field :label="__('candidates.filters.source')" name="source_type">
                                <select name="source_type" data-placeholder="{{ __('candidates.filters.source_placeholder') }}" class="w-full rounded-xl border border-aura-200/40 bg-white/80 px-3 py-2 text-sm">
                                    <option value="">{{ __('candidates.filters.source_placeholder') }}</option>
                                    @foreach($sources as $source)
                                        <option value="{{ $source }}" @selected(($filters['source_type'] ?? null) === $source)>{{ $source }}</option>
                                    @endforeach
                                </select>
                            </x-form-field>
                        </div>
                        ...
                        <div class="grid gap-2 md:grid-cols-2">
                            <button type="submit" class="rounded-xl bg-success-600 px-3 py-2 text-sm font-semibold text-white transition-weightless hover:bg-success-700">
                                {{ __('candidates.filters.apply') }}
                            </button>
                            <a href="{{ route('candidates.index', array_filter(['company_id' => $selectedCompanyId])) }}" class="rounded-xl border border-aura-300/40 bg-white/80 px-3 py-2 text-center text-sm font-semibold text-slate-700 transition-weightless hover:bg-white">
                                {{ __('candidates.filters.reset') }}
                            </a>
                        </div>

                    </form>
                    <div class="flex flex-wrap items-center gap-2 border-b border-white/60 bg-white/55 px-3 pb-3">
                        <x-modal id="candidate-export-modal" :title="__('ui.exports.modal.title')">
                            <x-slot:trigger>
                                <button type="button" class="rounded-xl border border-success-300/60 bg-success-50 px-3 py-2 text-sm font-semibold text-success-800 transition-weightless hover:bg-success-100/80">
                                    {{ __('ui.exports.modal.open_candidate') }}
                                </button>
                            </x-slot:trigger>

                            <form method="POST" action="{{ route('candidates.export') }}" class="space-y-4">
                                @csrf
                                @if(auth()->user()->isSuperadmin() && $selectedCompanyId)
                                    <input type="hidden" name="company_id" value="{{ $selectedCompanyId }}">
                                @endif

                                <input type="hidden" name="q" value="{{ (string) ($filters['q'] ?? '') }}">
                                <input type="hidden" name="job_id" value="{{ (string) ($filters['job_id'] ?? '') }}">
                                <input type="hidden" name="stage_id" value="{{ (string) ($filters['stage_id'] ?? '') }}">
                                <input type="hidden" name="status" value="{{ (string) ($filters['status'] ?? '') }}">
                                <input type="hidden" name="source_type" value="{{ (string) ($filters['source_type'] ?? '') }}">
                                <input type="hidden" name="date_from" value="{{ (string) ($filters['date_from'] ?? '') }}">
                                <input type="hidden" name="date_to" value="{{ (string) ($filters['date_to'] ?? '') }}">
                                <input type="hidden" name="application_id" value="{{ (string) ($filters['application_id'] ?? '') }}">

                                <x-form-field :label="__('ui.exports.fields.format')" name="format" required>
                                    <select name="format" required data-placeholder="{{ __('ui.exports.placeholders.format_required') }}" class="w-full rounded-xl border border-aura-200/40 bg-white/80 px-3 py-2.5 text-sm">
                                        <option value="">{{ __('ui.exports.placeholders.format_required') }}</option>
                                        @foreach(\App\Models\Export::formats() as $format)
                                            <option value="{{ $format }}" @selected((string) old('format') === (string) $format)>{{ strtoupper($format) }}</option>
                                        @endforeach
                                    </select>
                                </x-form-field>
                                ...
                                <button type="submit" class="rounded-xl bg-success-600 px-4 py-2 text-sm font-semibold text-white transition-weightless hover:bg-success-700">
                                    {{ __('ui.exports.modal.submit') }}
                                </button>
                            </form>
                        </x-modal>
                        @can('access-admin-pages')
                            <a href="{{ route('admin.exports.index', array_filter(['company_id' => $selectedCompanyId])) }}" class="rounded-xl border border-aura-300/40 bg-white px-3 py-2 text-sm font-semibold text-aura-800 transition-weightless hover:bg-aura-50">
                                {{ __('ui.exports.history.link_short') }}
                            </a>
                        @endcan
                    </div>

                    <div id="candidate-list-scroll" class="min-h-0 flex-1 overflow-y-auto px-3 py-3">
                        <div class="space-y-2">
                            @forelse($applications as $application)
                                @php
                                    $blind = \App\Http\Controllers\CandidateWorkspaceController::shouldMaskIdentity(
                                        $application->job,
                                        (string) optional($application->currentStage)->stage_key,
                                        (string) optional($application->currentStage)->stage_label
                                    );
                                    $maskedIdentifier = \App\Http\Controllers\CandidateWorkspaceController::maskedCandidateIdentifier((string) $application->id);
                                    $displayName = $blind
                                        ? __('candidates.detail.masked_identifier_value', ['identifier' => $maskedIdentifier])
                                        : (string) optional($application->candidate)->full_name;
                                    $query = array_filter([
                                ...
                                <a href="{{ route('candidates.index', $query) }}" @class([
                                    'js-candidate-link block rounded-xl border p-3 transition-weightless',
                                    (string) $selectedApplicationId === (string) $application->id
                                        ? 'border-aura-300 bg-aura-50/65 shadow-sm'
                                        : 'border-slate-200 bg-white/70 hover:border-aura-200 hover:bg-white',
                                ]) data-application-id="{{ $application->id }}">
                                    <div class="flex items-start justify-between gap-2">
                                        <div class="min-w-0">
                                            <p class="truncate text-sm font-semibold text-slate-900">{{ $displayName }}</p>
                                            <p class="truncate text-xs text-slate-600">{{ optional($application->job)->title }}</p>
                                        </div>
                                        <div class="flex flex-col items-end gap-1">
                                            @php
                                                $applicationStatusVariant = match ((string) $application->status) {
                                                    'hired' => 'success',
                                                    'rejected', 'withdrawn' => 'danger',
                                                    default => 'pending',
                                                };
                                            @endphp
                                            <x-badge :variant="$applicationStatusVariant">{{ __('candidates.list.status.'.$application->status) }}</x-badge>
                                            @if($blind)
                                    ...
                                    <div class="mt-2 flex items-center justify-between text-xs text-slate-600">
                                        <span class="truncate">{{ optional($application->currentStage)->stage_label }}</span>
                                        <span class="shrink-0">{{ __('candidates.list.last_activity') }}: {{ $application->updated_at?->diffForHumans() }}</span>
                                    </div>
                                    <div class="mt-1 flex items-center justify-between text-[11px] text-slate-500">
                                        <span>#{{ \Illuminate\Support\Str::before((string) $application->id, '-') }}</span>
                                        <span class="font-semibold text-aura-700">{{ __('candidates.list.open') }}</span>
                                    </div>
                                </a>
                            @empty
                                    ...
                                    <div class="flex flex-wrap items-center justify-between gap-3">
                                        <div>
                                            <div class="flex items-center gap-2">
                                                <h3 class="text-xl font-semibold text-slate-900">{{ $displayName }}</h3>
                                                @if($blind)
                                                    <x-badge>{{ __('candidates.detail.blind_mode') }}</x-badge>
                                                @endif
                                            </div>
                                        ...
                                        <div class="flex flex-wrap gap-2">
                                            ...
                                            <span @class([
                                                ...
                                                $analysisStatus === 'ready' => 'border-success-300/60 bg-success-50 text-success-800',
                                                $analysisStatus === \App\Services\Analysis\CandidateAnalysisService::ANALYSIS_INVALID_CV => 'border-danger-300/60 bg-danger-50 text-danger-800',
                                                $analysisStatus === 'insufficient_data' => 'border-primary-300/60 bg-primary-50 text-primary-800',
                                                $analysisStatus !== 'ready'
                                                    && $analysisStatus !== \App\Services\Analysis\CandidateAnalysisService::ANALYSIS_INVALID_CV
                                                    && $analysisStatus !== 'insufficient_data' => 'border-slate-200 bg-slate-100 text-slate-700',
                                            ])>
                                                {{ __('candidates.detail.analysis_status') }}: {{ $analysisStatusLabel }}
                                            </span>
                                            @if($cvSourceStatusLabel !== '')
                                                <span class="inline-flex items-center rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700">
                                                    {{ __('candidates.detail.cv_source_status') }}: {{ $cvSourceStatusLabel }}
                                                </span>
                                            @endif
                                    ...
                                    <div class="mt-3 grid gap-2 md:grid-cols-2 2xl:grid-cols-4">
                                        ...
                                        <form method="POST" action="{{ route('candidates.schedule-interview', ['application' => $selectedApplication->id]) }}" class="space-y-2 rounded-xl border border-slate-200 bg-white/80 p-2" data-interview-schedule-form>
                                            ...
                                            @foreach($query as $key => $value)
                                                <input type="hidden" name="{{ $key }}" value="{{ is_scalar($value) ? (string) $value : '' }}">
                                            @endforeach
                                            <select name="interview_type" data-placeholder="{{ __('interviews.fields.interview_type') }}" class="w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs">
                                                <option value="screening">{{ __('interviews.types.screening') }}</option>
                                                <option value="technical">{{ __('interviews.types.technical') }}</option>
                                                <option value="manager">{{ __('interviews.types.manager') }}</option>
                                                <option value="final">{{ __('interviews.types.final') }}</option>
                                            </select>
                                            ...
                                            <select name="timezone" required data-placeholder="{{ __('interviews.fields.timezone') }}" class="w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs">
                                                @foreach($timezones as $timezone)
                                                    <option value="{{ $timezone }}" @selected($timezone === config('app.timezone', 'UTC'))>{{ $timezone }}</option>
                                                @endforeach
                                            </select>
                                            ...
                                            <label class="flex items-center gap-2 text-[11px] text-slate-700">
                                                <input type="checkbox" name="admin_override_past" value="1">
                                                <span>{{ __('interviews.fields.admin_override_past') }}</span>
                                            </label>
                                            <button type="submit" class="w-full rounded-lg border border-primary-300/60 bg-primary-50 px-2 py-1.5 text-xs font-semibold text-primary-800 transition-weightless hover:bg-primary-100/80">
                                    ...
                                    <div class="rounded-xl border border-slate-200 bg-white p-4 lg:col-span-3">
                                        ...
                                        <div class="mt-3 space-y-2 text-sm text-slate-700">
                                            @php
                                                ...
                                                $skills = $cv ? collect($cv->parsed_skills ?? [])->take(8)->implode(', ') : __('candidates.detail.not_available');
                                                $formation = $cv && !empty($cv->parsed_education) ? ($cv->parsed_education[0]['degree_name'] ?? __('candidates.detail.not_available')) : __('candidates.detail.not_available');
                                                $experience = $cv && $cv->total_years_experience !== null ? number_format((float) $cv->total_years_experience, 1) . ' ans' : __('candidates.detail.not_available');
                                            @endphp
