                                            ...
                                            @foreach($group['applications'] as $app)
                                                ...
                                                <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm hover:border-aura-300">
                                                    ...
                                                    <div class="mt-4 flex gap-2">
                                                        ...
                                                        @else
                                                            <button type="button" onclick="alert('Aucun CV n\'est disponible pour ce candidat.')" class="flex-shrink-0 flex items-center justify-center rounded-lg bg-slate-50 px-2 py-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition" title="Pas de CV">
                                                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                ...
                                <div class="rounded-2xl border border-slate-200 bg-white/70 p-4">
                                    ...
                                    <div class="mb-4 grid gap-4 lg:grid-cols-4">
                                        ...
                                        <div class="rounded-xl border border-slate-200 bg-white p-4 lg:col-span-3">
                                            ...
                                            <div class="mt-4 flex flex-wrap gap-3 border-t border-slate-100 pt-3">
                                                @php
                                                    $cvDoc = $selectedApplication->candidate?->documents->first();
                                                    $cvUrl = $cvDoc ? \App\Http\Controllers\CandidateWorkspaceController::signedDocumentUrl($cvDoc) : null;
                                                @endphp
                                                @if($cvUrl)
                                                    <a href="{{ $cvUrl }}" target="_blank" class="inline-flex items-center gap-1.5 rounded-lg bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 border border-slate-200 hover:bg-slate-50 transition shadow-sm">
                                                    ...
                                                    <a href="{{ $cvUrl }}" download="CV_{{ $selectedApplication->candidate?->full_name ?? 'Candidat' }}.pdf" class="inline-flex items-center gap-1.5 rounded-lg bg-aura-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-aura-700 transition shadow-sm">
                                                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                            <path stroke-linecap="round" stroke-linejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                                                        </svg>
                                                        Télécharger CV
                                                ...
                                                @else
                                                    <button type="button" onclick="alert('Aucun CV n\'est disponible pour ce candidat.')" class="inline-flex items-center gap-1.5 rounded-lg bg-white px-3 py-1.5 text-xs font-semibold text-slate-400 border border-slate-200 hover:bg-slate-50 transition shadow-sm">
                                                        <svg class="h-4 w-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                            <path stroke-linecap="round" stroke-linejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" />
                                                        </svg>
                                                        Aperçu CV
                    ...
                    @forelse($jobs as $index => $job)
                        @php
                            $bgColors = [
                                ...
                                'rgba(241, 245, 249, 0.7)', // Gray
                                'rgba(243, 232, 255, 0.7)', // Purple
                                'rgba(220, 252, 231, 0.7)', // Green
                            ];
                            $bgColor = $bgColors[$index % count($bgColors)];
                        @endphp
                        <a href="{{ route('candidates.index', array_merge(request()->query(), ['job_id' => $job->id])) }}" class="relative block h-[10.5rem] w-full group drop-shadow-sm hover:drop-shadow-lg transition-all duration-300 hover:-translate-y-1">
                            
                            <!-- The Card Background (masked) -->
                            <div class="w-full h-full rounded-[1.75rem] p-6 backdrop-blur-md border border-white/60 transition-transform duration-300" 
                                 style="background-color: {{ $bgColor }}; -webkit-mask-image: radial-gradient(circle at right 28px bottom 28px, transparent 36px, black 37px); mask-image: radial-gradient(circle at right 28px bottom 28px, transparent 36px, black 37px);">
                                <div class="flex flex-col h-full justify-between">
                                    <div>
                                        <h3 class="font-bold text-slate-900 text-lg leading-tight line-clamp-1">{{ $job->title }}</h3>
                                        <p class="text-xs text-slate-600 font-medium mt-1">Total Interaction</p>
                                    </div>
                            ...
                            <!-- The Button -->
                            <div class="text-white rounded-full flex items-center justify-center transition-transform duration-300 group-hover:scale-110" 
                                 style="position: absolute; bottom: 4px; right: 4px; width: 48px; height: 48px; background-color: #0f172a;">
                                    ...
                                    <tr class="transition hover:bg-slate-50">
                                        ...
                                        <td class="whitespace-nowrap px-6 py-4 text-right">
                                            <div class="flex items-center justify-end gap-2">
                                                ...
                                                @else
                                                    <button type="button" onclick="alert('Aucun CV n\'est disponible pour ce candidat.')" class="inline-flex items-center justify-center rounded-lg bg-white p-1 text-slate-400 hover:text-slate-600 hover:bg-slate-50 border border-transparent transition" title="Pas de CV">
                                                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                            <path stroke-linecap="round" stroke-linejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" />
                                                        </svg>
                                                    </button>
